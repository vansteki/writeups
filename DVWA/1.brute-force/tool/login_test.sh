...
# login at login page
curl -v -L -c cookies.txt -b cookies.txt -X POST "http://localhost:8086/login.php" --data-raw "username=${USER}&password=${LOGIN_PASSWORD}&Login=Login&user_token=${CSRF}"

# enter username and password in brute force page: /vulnerabilities/brute/
curl -L -v -c cookies.txt -b cookies.txt "http://localhost:8086/vulnerabilities/brute/?username=${USER}&password=${LOGIN_PASSWORD}&Login=Login#"
